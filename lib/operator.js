...
const JOINER = ', ';
const createUpdates = (updates, columns) => updates
  .filter(update => columns.indexOf(update) !== -1)
  .map(update => `${update}=VALUES(${update})`)
  .join(JOINER);
...
proto.insert = function* (table, rows, options) {
  ...
  const sql = this.format('INSERT INTO ??(??) VALUES' + strs.join(JOINER), params) + (
    updates.length === 0
      ? ''
      : typeof updates === 'string'
        ? ` ON DUPLICATE KEY UPDATE ${updates}`
        : ` ON DUPLICATE KEY UPDATE ${createUpdates(updates, options.columns)}`
  );
